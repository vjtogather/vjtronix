  ...
  return (
    <article className="group overflow-hidden rounded-2xl border bg-card transition hover:-translate-y-0.5 hover:shadow-lg">
      <Link className="relative block aspect-[16/9] overflow-hidden bg-gradient-to-br from-sky-500/20 to-violet-500/15" href={`/blog/${blog.slug}`}>
        {blog.thumbnail ? <Image alt="" className="object-cover transition duration-300 group-hover:scale-105" fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" src={blog.thumbnail} unoptimized /> : <span className="grid h-full place-items-center text-sm font-semibold text-sky-700 dark:text-sky-300">VJtronix</span>}
      ...
      <div className="p-5">
        ...
        <p className="mt-3 line-clamp-2 text-sm leading-6 text-muted-foreground">{blog.excerpt || "Explore this VJtronix engineering article."}</p>
        <div className="mt-5 flex flex-wrap items-center gap-4 text-xs text-muted-foreground"><span>By {blog.author.name || "VJtronix"}</span><span className="inline-flex items-center gap-1.5"><CalendarDays className="size-3.5" />{formatDate(blog.updatedAt)}</span><span className="inline-flex items-center gap-1.5"><Clock3 className="size-3.5" />{blog.readingTime} min read</span></div>
      </div>
    </article>
  );
}
